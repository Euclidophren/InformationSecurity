...
class AES(object):
    ...
    def __init__(self, key):
        ...
        self._key = [key[i:i+4] for i in range(0, len(key), 4)]
    ...
    def encrypt_block(self, plaintext):
        ...
        add_round_key(plain_state, self._key)
    ...
    def decrypt_block(self, ciphertext):
        ...
        add_round_key(cipher_state, self._key)
...
if __name__ == '__main__':
    key = bytes("dyftvghdsjofgios", encoding='utf-8').hex()
    filename = input("Enter filename: ")

    data = readFromFile(filename)

    encrypted = encrypt(key, data)
    writeToFile(str(encrypted), filename + ".encoded")
    encrypted = readFromFile(filename + ".encoded")
    decrypted = decrypt(key, encrypted)
    writeToFile(str(decrypted), filename + ".decoded")